package dev.rvbsm.ilmater.player;
...
public final class PersonalRules {
    ...
    public static final Map<String, GameRules.Key<?>> SUPPORTED_KEYS = SUPPORTED_RULES.stream()
    ...
    public Map<GameRules.Key<?>, GameRules.Rule<?>> getRules() {
        return this.rules.entrySet()
            .stream()
            .filter(entry -> IlmaterSettings.personalRules.contains(entry.getKey()))
    ...
    public <T extends GameRules.Rule<T>> Optional<T> get(GameRules.Key<T> key) {
        if (!IlmaterSettings.personalRules.contains(key) || !this.appliedRules.contains(key)) {
    ...
    public <T extends GameRules.Rule<T>> Optional<T> addRule(GameRules.Key<T> key) {
        if (!IlmaterSettings.personalRules.contains(key)) {
            return Optional.empty();
        }
